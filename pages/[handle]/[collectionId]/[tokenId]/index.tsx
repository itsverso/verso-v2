...
const Token = (props: Props) => {
  ...
  const { data, isLoading, mutate } = useGetTokenDetails(collectionId, tokenId);
  ...
  const executeBuy = async () => {
    if (data && !isLoading && user?.wallet) {
      setLoading(true);
      try {
        const market = getMarketContractInstance(data.market, InfuraProvider);
        const price = await market.getBuyPrice(collectionId, tokenId, 1);
        const provider = await user?.wallet?.getEthersProvider();
        const signer = provider?.getSigner();
        const collectionContract = getCollectionInstance(collectionId, signer);
        const buy = await collectionContract.collect(
          tokenId,
        ...
        await buy.wait();
        await createPost(props.collectionId, user?.wallet?.address, tokenId);
    ...
    <main className="flex flex-col md:flex-row min-w-screen min-h-screen pt-10 md:pt-0 px-6 md:px-0">
      ...
      <div className="relative flex flex-col justify-between md:w-1/3 md:h-screen md:px-10 pt-6 md:pt-12 pb-10">
        <div className="">
          ...
          <p className="mt-8 text-4xl font-hedvig">{data?.metadata?.title}</p>
        ...
        <div className="">
          <p className="mt-4">
            Collected <span className="font-bold">{data?.supply}</span> times
          </p>
          ...
          <div className="w-full h-16 bg-zinc-100 my-4 p-4 flex flex-col center justify-center">
            ...
            <p className="text-sm">{props.collectionId}</p>
          </div>
          <div className="w-full h-16 bg-zinc-100 my-4 p-4 flex flex-col center justify-center">
            <p className="text-sm font-bold">Arweave ID</p>
            <p className="text-sm">
              {data?.metadataURI.split("https://arweave.net/")[1]}
            </p>
          </div>
