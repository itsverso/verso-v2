...
const TokenId: NextPage = (props: any) => {
  ...
  const router = useRouter();
  const { data, error, isLoading, mutate } = useGetTokenDetails(collection, id);
  ...
  const [protocolEarningsArray, setProtocolEarningsArray] = useState<number[]>([
    0,
  ]);
  ...
  const executeBuy = () => {
    ...
    let protocolEarning =
      protocolEarningsArray[protocolEarningsArray.length - 1] + protocolFee;
    ...
    <main className="flex flex-row min-w-screen min-h-screen">
      ...
      <div className="relative flex flex-col justify-between w-1/3 h-screen px-10 pt-12 pb-10">
        <div className="">
          <div className="">
            {data ? (
              <CreatorCard handle={data?.token?.rawMetadata.creator} />
            ) : null}
          </div>
          <button
        ...
        <div className="">
          <p className="mt-4">
            Collected <span className="font-bold">{tokenSupply}</span> times
          </p>
          <div className="w-full h-16 bg-zinc-100 my-4 p-4 flex flex-col center justify-center">
            ...
            <p className="text-base font-bold">
              ...
              ETH - {parseEthOutputInUSD(getBuyPriceAfterFee(1)) + " "}
              USD
            </p>
          </div>
          <div className="w-full h-16 bg-zinc-100 mt-4 p-4 flex flex-col center justify-center">
            ...
            <p className="text-base font-bold">
              {parseEthOutput(
                creatorEarningsArray[creatorEarningsArray.length - 1]
              )}{" "}
              ETH -{" "}
              {parseEthOutputInUSD(
                creatorEarningsArray[creatorEarningsArray.length - 1]
          ...
          <div className=" w-full flex relative py-8 ">
            <p className="text-base italic text-left">
              ...
              This means you can buy or sell anytime you want. The more people
              buy, the higher the price and viceversa. This makes ART more
              accessible and fun.
            </p>
          </div>
