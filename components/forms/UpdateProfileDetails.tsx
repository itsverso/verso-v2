...
type Props = {
	...
	profile: Profile;
	onUpdateComplete: (profile: Profile) => void;
};
...
export function UpdateProfileDetailsForm(props: Props) {
	...
	const [warpcast, setwarpcast] = useState<string | undefined>(
		props.profile.metadata.warpcast ?? undefined
	);
	...
	const handleUpdateProfile = async () => {
		const profile = await updateProfile(props.wallet, props.profile, {
			name,
			handle: props.profile.metadata.handle,
			description,
			image,
			imageUrl,
			mimeType,
		...
		props.onUpdateComplete(profile);
	};
	...
	const profileImage = imageUrl || image;
